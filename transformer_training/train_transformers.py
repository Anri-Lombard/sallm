...
class GPT(nn.Module):
    ...
    def forward(
        ...
        # Create dynamic pattern of block masks based on layer count
        # Base pattern: [long_bm, short_bm, short_bm]
        base_pattern = [
            long_bm,
            short_bm,
            short_bm,
        ...
        # Create encoder block masks by cycling through pattern as needed
        encoder_block_masks = []
        ...
        # Create decoder block masks (reverse of encoder pattern)
        decoder_block_masks = []
...
class Hyperparameters:
    def __init__(self, args):
        ...
        # Calculate total tokens by reading headers of all training files
        import glob
        import torch
...
def main():
    ...
    if master_process:
        wandb.init(
            ...
            config={
                ...
                "val_loss_every": hparams.val_loss_every,
    ...
    # init the optimizer(s) with configurable learning rates and hyperparameters
    adam_params = [
    ...
    def get_lr(it: int):
        t = 1 - it / hparams.num_iterations  # time remaining in training
        assert 1 >= t >= 0
        w = min(t / hparams.cooldown_frac, 1.0)  # 1 -> 0
    ...
    for step in range(train_steps + 1):
        ...
        # --------------- VALIDATION SECTION -----------------
        if last_step or (
            hparams.val_loss_every > 0 and step % hparams.val_loss_every == 0
        ):
            # Stop the clock
            torch.cuda.synchronize()
        ...
        for input_seq, target_seq in zip(
            ...
            if master_process:
                running_loss = running_loss * step / (step + 1) + loss.item() / (
                    step + 1
                )
                wandb.log(
                    {

...
class ValueEmbedding(nn.Module):
    def __init__(self, num_embeddings: int, embedding_dim: int):
        ...
        self.embed = nn.ModuleList(
            [nn.Embedding(num_embeddings, embedding_dim) for _ in range(3)]
        )
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        x = norm(x)
        logits = (
            lm_head_fp8(x, self.lm_head.weight) if self.training else self.lm_head(x)
        )
...
class Hyperparameters:
    # data
    train_files = "/scratch/lmbanr001/masters/dataset/data_balanced/custom_train_*.bin"  # input .bin to train on
    val_files = "/scratch/lmbanr001/masters/dataset/data_balanced/custom_val_*.bin"  # input .bin to eval validation loss on
    val_tokens = 10383360  # how many tokens of validation data? it's important to keep this fixed for consistent comparisons
    # optimization
    batch_size = 64 * 1024  # batch size in tokens
    num_iterations = 1889  # number of iterations for 3 epochs
...
optimizer2 = Muon(
    hidden_matrix_params, lr=0.05, momentum=0.95, rank=rank, world_size=world_size
)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(
            f"step:{step}/{train_steps} val_loss:{val_loss:.4f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms / (timed_steps - 1):.2f}ms",
            console=True,
        )
        model.train()

...
class Hyperparameters:
    # data
    train_files = "/scratch/lmbanr001/masters/dataset/filtered_data_binary_balanced/balanced_train_*.bin"  # input .bin to train on
    val_files = "/scratch/lmbanr001/masters/dataset/filtered_data_binary_balanced/balanced_val_*.bin"  # input .bin to eval validation loss on
    # TODO: change
    val_tokens = (
        30_953_790 // 8192
    ) * 8192  # how many tokens of validation data? it's important to keep this fixed for consistent comparisons
    # optimization
    batch_size = 64 * 1024  # batch size in tokens
    # TODO: change
    train_tokens = (123_499_818 // 8192) * 8192
    num_iterations = (train_tokens * 10) // (
        64 * 1024
    )  # for 3 epochs TODO: with synthetic data
    cooldown_frac = 0.4  # fraction of training spent cooling down the learning rate
    # evaluation and logging
    val_loss_every = (
        200  # every how many steps to evaluate val loss? 0 for only at the end
    )
    # implementation
    seq_len = 2 * 1024  # FlexAttention sequence length
    save_checkpoint = True
    wandb_project = "sallm"
    wandb_entity = "anri-m-lombard"
    wandb_name = "12.5M_balanced"
...
model = GPT(vocab_size=50257, num_layers=6, num_heads=8, model_dim=512).cuda()
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        val_loader = distributed_data_generator(
            args.val_files, val_bs, rank, world_size
        )
        val_loss = 0
        with torch.no_grad():
            ...
            wandb.log(
                {
                    ...
                    "val/perplexity": val_perplexity,  # Log perplexity
                    "train/step": step,
        ...
        print0(
            f"step:{step}/{train_steps} val_loss:{val_loss:.4f} val_perplexity:{val_perplexity:.4f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms / (timed_steps - 1):.2f}ms",
            console=True,
        )
        model.train()

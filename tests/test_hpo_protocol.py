...
def test_seed42_ranking_uses_frozen_tie_breaks(tmp_path: Path) -> None:
    trials = [
        write_trial(tmp_path, candidate=candidate, seed=42, metric=0.5)
        for candidate in ("b0", "b1", "b2")
    ]
    result = run(
        str(PROTOCOL),
        "rank",
...
def test_confirmation_ranking_reuses_seed42_and_requires_confirmation_stages(
    ...
    trials = [
        write_trial(tmp_path, candidate="b0", seed=42, metric=0.4),
        write_trial(tmp_path, candidate="b0", seed=13, metric=0.5, stage="confirm"),
        write_trial(tmp_path, candidate="b0", seed=87, metric=0.6, stage="confirm"),
    ]
    result = run(
        str(PROTOCOL),
        "rank",
    ...
    assert result.returncode == 0, result.stderr
    row = json.loads(result.stdout)["ranking"][0]
    assert row["candidate"] == "b0"
    assert row["mean"] == 0.5
    assert row["sample_sd"] == 0.09999999999999998
